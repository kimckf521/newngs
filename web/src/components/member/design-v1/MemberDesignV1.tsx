        ...
        {courses.map((c) => (
          <Card key={c.id} className="overflow-hidden">
            ...
            <div className="p-5">
              ...
              <p className="mt-1 text-xs text-slate-400">{c.modules.length} {t.modulesWord} · {c.progress}% {t.completeWord}</p>
              ...
              <ul className="mt-4 space-y-1.5">
                {c.modules.slice(0, 3).map((mod) => (
                  <li key={mod.title} className="flex items-center gap-2 truncate text-[13px] text-slate-500">
              ...
              <div className="mt-5 flex gap-2.5">
                <GradientButton className="flex-1">{t.continue}<Icon name="play" className="h-3.5 w-3.5" /></GradientButton>
                <SoftButton>{t.viewModules}</SoftButton>
              </div>
            </div>
          </Card>
        ))}
